...
def get_all_lessons():
    ...
    for subject_name in subjects:
        subtopics = loader.get_subtopics(subject_name)
...
def get_lesson_plans(subject, subtopic):
    ...
    if isinstance(lessons, list):
        ...
        return {}s including dashboard, subject management,
lesson administration, and system oversight.
"""
...
def admin_lessons():
    ...
    if filtered_view:
        ...
        data_root_path = os.path.join(os.path.dirname(__file__), '..', 'data')
        loader = DataLoader(data_root_path)
        if not loader.validate_subject_subtopic(subject_filter, subtopic_filter):
...
def admin_create_lesson():
    ...
    try:
        ...
        print(f"Error loading lesson creation form: {e}")
        return f"Error: {e}", 500
...
def admin_reorder_lessons(subject, subtopic):
    ...
    try:
        ...
        result = admin_service.reorder_lessons(subject, subtopic, lesson_order)
...
def admin_quiz_editor(subject, subtopic):
    ...
    try:
        ...
        files_exist = data_service.validate_subject_subtopic(subject, subtopic)

        # Load quiz data and question pool (will be None if files don't exist)
        quiz_data = data_service.get_quiz_data(subject, subtopic)
        pool_questions = data_service.get_question_pool_questions(subject, subtopic)
...
def admin_quiz_initial(subject, subtopic):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            result = admin_service.save_quiz_questions(subject, subtopic, questions, "initial")
            ...
            print(f"Error saving initial quiz data: {e}")
            return jsonify({"error": str(e)}), 500
...
def admin_quiz_pool(subject, subtopic):
    ...
    if request.method == "GET":
        try:
            data_service = get_data_service()
            pool_questions = data_service.get_question_pool_questions(subject, subtopic)
            return jsonify({"questions": pool_questions if pool_questions else []})
        except Exception as e:
            print(f"Error loading question pool data: {e}")
            return jsonify({"error": str(e)}), 500
        ...
        try:
            ...
            result = admin_service.save_quiz_questions(subject, subtopic, questions, "pool")
            ...
            print(f"Error saving question pool data: {e}")
            return jsonify({"error": str(e)}), 500


# ============================================================================
# SUBTOPICS MANAGEMENT ROUTES
# ============================================================================
    ...
    try:
        ...
        for subject_id, subject_info in subjects.items():
            ...
            if subject_config:
                ...
                config_subtopics = subject_config.get("subtopics", {})
...
def admin_toggle_override():
    ...
    try:
        ...
        if request.method == "POST":
            result = admin_service.toggle_admin_override()
            return jsonify(result)
        else:
        ...
        print(f"Error with admin override: {e}")
        return jsonify({"error": str(e)}), 500
...
def admin_clear_cache():
    ...
    try:
        # Get data service and clear its cache
        data_service = get_data_service()
        data_service.clear_cache()

        print("DataLoader cache cleared successfully")
        return jsonify(
            {
                "success": True,
